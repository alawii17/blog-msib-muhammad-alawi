        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST" class="mt-5"
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" required
                    ...
                    value="{{ old('name') ?? $user->name }}" placeholder="Enter auhtor name">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="username" name="username" required
                    ...
                    value="{{ old('username') ?? $user->username }}" placeholder="Enter user username">
                @error('name')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="email" name="email" required
                    ...
                    value="{{ old('email') ?? $user->email }}" placeholder="Enter auhor email">
                @error('email')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
